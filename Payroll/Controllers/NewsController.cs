...
public class NewsController : BaseController
{
  ...
  [HttpGet]
  [Route("/admin/news")]
  public async Task<IActionResult> Index(string keyword, NewsSelectedOptions options, PaginationOptions pagination)
  {
    try
    {
      ...
      var uniqueUpdateList = newsList
        .Where(vm => string.IsNullOrEmpty(options.SelectedActivity) ||
      ...
      IList<NewsViewModel> list = newsList.OrderByDescending(s => s.PublishedAt).ToList();

      var paginationList = PaginationList<NewsViewModel>.Create(list, pagination.PageNumber, pagination.PageSize);
